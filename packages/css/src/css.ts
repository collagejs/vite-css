...
const cssInjectedDictionary = '{cjcss:CSS_MAP}';
const cssMap: Record<string, string[]> = JSON.parse(cssInjectedDictionary);
const cssFileMap: Record<string, { count: number, linkElement: HTMLLinkElement | undefined }> = {};
const viteBase = import.meta.env.BASE_URL;
let autoLinksMounted = false;

export function cssMountFactory(entryPoint: string, options?: CssMountFactoryOptions) {
    ...
    async function mount(this: MountBindOptions | undefined) {
        if (!observer) {
        ...
        if (cssFileNames.length > 0) {
            ...
            base += base.endsWith('/') ? '' : '/';
            const cssPromises = cssFileNames.map(css => mountCssFile(css, base, opts.loadTimeout));
 ...
 */
function mountCssFile(cssFileName: string, base: string, loadTimeout: number) {
    return new Promise<LinkLoadResult>((rslv, _rjct) => {
        let map = cssFileMap[cssFileName] ?? { count: 0, linkElement: undefined };
        ...
        // network request might show up in the Network tab of the browser's developer tools.
        if (!firstTimeLoading) {
            rslv({ status: 'ok' });
            return;
        }
...
function unmountCssFile(cssFileName: string) {
    ...
    if (!map.linkElement) {
        logger.warn('A request to unmount CSS file %s was made, but its link element is undefined.', cssFileName);
        return;
    }
